  ...
  stages { // Defines the different stages of the pipeline
    ...
    stage ('docker run container') { // Defines the 'docker run container' stage
      ...
      //  sh 'sudo docker rm  app2 || true'  Removes the container with the name 'app' if it exists
        sh 'sudo docker run --name app_$(date +%s) -d -p 9003:8080 javulna-0.1' // Runs a new Docker container named 'app' based on the 'javulna-0.1' image, with port mapping from 8080 to 9000, in detached mode (-d), and allocates a pseudo-TTY (-it)
     }
   }
 }
}
